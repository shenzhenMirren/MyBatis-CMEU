...
public class MapperMateriHaveJDBC extends MapperFactory {
	 ...
	 * @param isAssist
	 */
	public MapperMateriHaveJDBC(boolean isAssist) {
		super();
				...
				if (cvf.get(i).getCheck() != false) {
						...
						if (sa.getPrimaryKey() != null && !"".equals(sa.getPrimaryKey())) {
							for (int j = 0; j < cvf.size(); j++) {
								if (sa.getPrimaryKey().equals(cvf.get(j).getConlumn())) {
									result.append("        <id column=\"" + sa.getClassName()
											+ cvf.get(j).getPropertyName() + "\"");
									if (cvf.get(j).getJdbcType() != null) {
										result.append(" jdbcType=\"" + cvf.get(j).getJdbcType() + "\"");
									}
									result.append(" property=\"" + cvf.get(j).getPropertyName() + "\" />\r\n");

									if (falg) {
										// 记录用于给多对一天添加association
										oneInMany.append("            <id column=\"" + sa.getClassName()
												+ cvf.get(j).getPropertyName() + "\"");
										if (cvf.get(j).getJdbcType() != null) {
											oneInMany.append(" jdbcType=\"" + cvf.get(j).getJdbcType() + "\"");
										}
										oneInMany.append(" property=\"" + cvf.get(j).getPropertyName() + "\" />\r\n");
									}
								}
							}
						}
					...
					if (cvf.get(i).getConlumn() != null) {
						if (cvf.get(i).getConlumn().equals(sa.getPrimaryKey())) {
							...
							result.append(
									"        <id column=\"" + sa.getClassName() + cvf.get(i).getPropertyName() + "\"");
							if (cvf.get(i).getJdbcType() != null) {
								result.append(" jdbcType=\"" + cvf.get(i).getJdbcType() + "\"");
							}
							result.append(" property=\"" + cvf.get(i).getPropertyName() + "\" />\r\n");
							if (falg) {
								oneInMany.append("            <id column=\"" + sa.getClassName()
										+ cvf.get(i).getPropertyName() + "\"");
								if (cvf.get(i).getJdbcType() != null) {
									oneInMany.append(" jdbcType=\"" + cvf.get(i).getJdbcType() + "\"");
								}
								oneInMany.append(" property=\"" + cvf.get(i).getPropertyName() + "\" />\r\n");
							}
						}
					} else {
	...
	private String createSqlNoAS(List<SuperAttribute> supAttr) {
		...
		for (SuperAttribute Attr : supAttr) {
			...
			for (int i = 0; i < cvf.size(); i++) {
				...
				if (i == 0) {
					result.append("        " + Attr.getTableName() + "." + cvf.get(i).getConlumn() + " as "
							+ Attr.getClassName() + cvf.get(i).getPropertyName());
				} else {
					result.append(" ," + Attr.getTableName() + "." + cvf.get(i).getConlumn() + " as "
							+ Attr.getClassName() + cvf.get(i).getPropertyName());
				}
	...
	private String getRowCountNoAs(List<SuperAttribute> supAttr, String asSpace, String espace) {
		...
		for (SuperAttribute attr : supAttr) {
			result.append(
					"    <select id=\"get" + attr.getClassName() + "RowCount\" resultType=\"java.lang.Long\">\r\n");
			if (attr.getPrimaryKey() != null) {
				result.append(
						"        select count(" + attr.getPrimaryKey() + ") from " + attr.getTableName() + "\r\n");
			} else {
	...
	private String createMySqlAndPostgrePage(List<SuperAttribute> supAttr, String asSpace) {
		...
		for (int i = 0; i < supAttr.size(); i++) {
			...
			result.append(
					"        <if test=\"rowSize !=null\"> LIMIT #{rowSize,jdbcType=INTEGER} <if test=\"startRow !=null\"> OFFSET #{startRow,jdbcType=INTEGER}</if></if>\r\n");
			result.append("    </select> \r\n\r\n");
		}
	...
	private String createSqlServerPage(List<SuperAttribute> supAttr, String asSpace) {
		...
		for (int i = 0; i < supAttr.size(); i++) {
			...
			result.append("        (\r\n");
			result.append(
					"            select <if test=\"distinct !=null\">${distinct}</if> ROW_NUMBER() over(<choose><when test=\"order!=null\">${order}</when><otherwise>order by "
							+ supAttr.get(i).getTableName() + "." + supAttr.get(i).getPrimaryKey()
			...
			result.append("                <if test=\"startRow!=null\">and page &gt; #{startRow,jdbcType=INTEGER} </if>\r\n");
			result.append("                <if test=\"endRow!=null\">and page &lt;= #{endRow,jdbcType=INTEGER} </if>\r\n");
			result.append("            </when>\r\n");
			result.append("            <otherwise>\r\n");
			result.append("                <choose>\r\n");
			result.append(
					"                    <when test=\"startRow!=null\">where page &gt; #{startRow,jdbcType=INTEGER} <if test=\"endRow!=null\">and page &lt;= #{endRow,jdbcType=INTEGER} </if></when>\r\n");
			result.append(
					"                    <otherwise><if test=\"endRow!=null\">where page &lt;= #{endRow,jdbcType=INTEGER}</if></otherwise>\r\n");
			result.append("                </choose>\r\n");
			result.append("            </otherwise>\r\n");
			result.append("        </choose>\r\n");
			result.append("    </select> \r\n\r\n");
	...
	private String createOraclePage(List<SuperAttribute> supAttr, String asSpace) {
		...
		for (int i = 0; i < supAttr.size(); i++) {
			...
			result.append("                <choose>\r\n");
			result.append(
					"                    <when test=\"startRow!=null\">where page &gt; #{startRow,jdbcType=INTEGER} <if test=\"endRow!=null\">and page &lt;= #{endRow} </if></when>\r\n");
			result.append(
					"                    <otherwise><if test=\"endRow!=null\">where page &lt;= #{endRow,jdbcType=INTEGER}</if></otherwise>\r\n");
			result.append("                </choose>\r\n");
			result.append("            </otherwise>\r\n");
	...
	private String getSelectListNoAs(List<SuperAttribute> supAttr, String asSpace, String espace) {
		...
		for (int i = 0; i < supAttr.size(); i++) {
			...
			for (String tbn : fromList) {
				result.append("        inner join " + tbn + " on " + supAttr.get(i).getTableName() + "."
						+ supAttr.get(i).getPrimaryKey() + "=" + tbn + "." + supAttr.get(i).getPrimaryKey() + "\r\n");
			}
			result.append("    </select> \r\n\r\n");
		}
		return result.toString();
				...
				for (int j = 0; j < cvf.size(); j++) {
					if (cvf.get(j).getConlumn() != null) {
						if (supAttr.get(i).getPrimaryKey().equals(cvf.get(j).getConlumn())) {
							...
							if (selectJDBCType(supAttr.get(i).getAttributes(),
									supAttr.get(i).getPrimaryKey()) != null) {
								result.append(",jdbcType=" + selectJDBCType(supAttr.get(i).getAttributes(),
										supAttr.get(i).getPrimaryKey()));
							}
							result.append("}\r\n");
		...
		for (SuperAttribute sa : supAttr) {
			if (sa.getPrimaryKey() != null) {
				...
				if (javaIdType != null) {
					...
					if (selectJDBCType(sa.getAttributes(), sa.getPrimaryKey()) != null) {
						result.append(",jdbcType=" + selectJDBCType(sa.getAttributes(), sa.getPrimaryKey()));
					}
					result.append("}\r\n");
	 ...
	 * @return
	 */
	public String selectJDBCType(List<AttributeCVF> cvfs, String str) {
		for (int i = 0; i < cvfs.size(); i++) {
			if (cvfs.get(i).getConlumn() != null) {
				if (str.equals(cvfs.get(i).getConlumn())) {
